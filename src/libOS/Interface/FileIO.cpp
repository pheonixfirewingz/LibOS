...
extern std::string platformGetHomePath();
...
std::string getCorrectPath(_in_ const std::string path)
{
    ...
    pos = corrected_path.find("$[home]");                 // Searches for "$[home]" string in the path
    if (pos != std::string::npos)                         // If found
        corrected_path.replace(pos, std::string("$[home]").length(),
                               platformGetHomePath());    // Replaces it with the home path
